...
export async function deployFullSuiteFixture() {
  ...
  const tokenImplementation = await ethers.deployContract('TokenDLT', deployer);
  ...
  await token.connect(tokenAgent).mint(aliceWallet.address, 1, 1, 1000);
  await token.connect(tokenAgent).mint(bobWallet.address, 1, 2, 500);
