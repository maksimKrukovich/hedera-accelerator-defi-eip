...
async function deployFixture() {
  ...
  const notOwner = wallet.connect(ethers.getDefaultProvider());
...
describe('ERC721Metadata', () => {
  describe('.getMetadata(uint256)', () => {
    ...
    describe('when there is metadata', () => {
      it('should return metadata list', async () => {
        const { token } = await deployWithMintedTokenFixture();
  ...
  describe('.setMetadata(uint256,string,string)', () => {
    describe('when there is no token', () => {
      it('should revert', async () => {
        ...
        await expect(token['setMetadata(uint256,string,string)'](1, 'color', 'blue')).to.be.reverted;
      });
    ...
    describe('when there is token', () => {
      describe('when it is not the token owner', () => {
        it('should revert', async () => {
          const { token, notOwner } = await deployWithMintedTokenFixture();
  ...
  describe('.freezeMetadata()', () => {
    describe('when it is not the owner', () => {
      it('should revert', async () => {
        const { token, notOwner } = await deployWithMintedTokenFixture();
        await expect(token.connect(notOwner).freezeMetadata(0)).to.be.reverted;
      });
    });
  ...
  describe('.mint(address,string)', () => {
    describe('when minter is not the owner', () => {
      it('should revert', async () => {
        const { token, notOwner }  = await deployFixture();
        await expect(token.connect(notOwner)['mint(address,string)'](notOwner.getAddress(), "ipfs://someLocation")).to.be.reverted;
      });
    });
  ...
  describe('.setTokenURI()', () => {
    describe('when it is not the token owner', () => {
      it('should revert', async () => {
        const { token, notOwner } = await deployWithMintedTokenFixture();
        await expect(token.connect(notOwner).setTokenURI(0, 'ipfs://newlocation')).to.be.reverted;
      });
    });
  ...
  describe('.setCollectionMetadata()', () => {
    describe('when it is not the token owner', () => {
      it('should revert', async () => {
        const { token, notOwner } = await deployWithMintedTokenFixture();
        await expect(token.connect(notOwner).setCollectionMetadata(['type'], ['collection'])).to.be.reverted;
      });
    });
    ...
    describe('when it is the token owner', () => {
      describe('when a empty array is passed', () => {
        it('should revert', async () => {
          const { token, owner } = await deployWithMintedTokenFixture();
