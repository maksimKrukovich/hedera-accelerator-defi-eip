...
import "./HTSNonFungibleToken.sol";
/**
 * @title ERC721Metadata
 * @author Hashgraph
 * @notice ERC721 custom contract to handle onchain metadata
 */
contract ERC721MetadataHTS is ERC721, ERC721URIStorage, Ownable, HTSNonFungibleToken {
     ...
     */
    mapping(uint256 => bool) internal isMetadataFrozen;
    ...
    modifier whenUnfrozen(uint256 _tokenId) {
        require(!isMetadataFrozen[_tokenId], "ERC721Metadata: token metadata can no longer be modified");
        _;
    }
    ...
    constructor (string memory _name, string memory _symbol) 
        ...
        Ownable(_msgSender())
        HTSNonFungibleToken(_name, _symbol) {}
    ...
    function freezeMetadata(uint256 _tokenId) external onlyOwner whenUnfrozen(_tokenId) {
        isMetadataFrozen[_tokenId] = true;
    }
