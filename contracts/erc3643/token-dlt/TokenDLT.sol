...
import "./ITokenDLT.sol";
import "../../onchainid/interface/IIdentity.sol";
import "./TokenDLTStorage.sol";
import "../roles/AgentRoleUpgradeable.sol";
import "./dlt/extensions/DLTERC20FIFO.sol";
import "./dlt/DLT.sol";

contract TokenDLT is ITokenDLT, AgentRoleUpgradeable, TokenDLTStorage, DLTERC20FIFO {
     ...
     */
    function batchMint(address[] calldata _toList, int64[] calldata _mainIds, int64[] calldata _subIds, uint256[] calldata _amounts) external override {
        for (uint256 i = 0; i < _toList.length; i++) {
            mint(_toList[i], _mainIds[i], _subIds[i], _amounts[i]);
        }
    }
     ...
     */
    function batchBurn(address[] calldata _userAddresses, int64[] calldata _mainIds, int64[] calldata _subIds, uint256[] calldata _amounts) external override {
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            _burn(_userAddresses[i], _mainIds[i], _subIds[i], _amounts[i]);
        }
    }
    ...
    function recoveryAddress(
        ...
        require(super.balanceOf(_lostWallet) != 0, "no tokens to recover");
        IIdentity _onchainID = IIdentity(_investorOnchainID);
        bytes32 _key = keccak256(abi.encode(_newWallet));
        if (_onchainID.keyHasPurpose(_key, 1)) {
            uint256 investorTokens = super.balanceOf(_lostWallet);
    ...
    function transfer(address _to, uint256 _amount) public override whenNotPaused returns (bool) {
        ...
        require(_amount <= super.balanceOf(msg.sender) - (_frozenTokens[msg.sender]), "Insufficient Balance");
    ...
    function forcedTransfer(
        ...
        require(super.balanceOf(_from) >= _amount, "sender balance too low");
        uint256 freeBalance = super.balanceOf(_from) - (_frozenTokens[_from]);
     ...
     *  @dev See {IToken-burn}.
     */
    function burn(address _userAddress,  int64 _mainId, int64 _subId,  uint256 _amount) public onlyAgent {
        require(super.balanceOf(_userAddress) >= _amount, "cannot burn more than balance");
        uint256 freeBalance = super.balanceOf(_userAddress) - _frozenTokens[_userAddress];
        if (_amount > freeBalance) {
            uint256 tokensToUnfreeze = _amount - (freeBalance);
            _frozenTokens[_userAddress] = _frozenTokens[_userAddress] - (tokensToUnfreeze);
            emit TokensUnfrozen(_userAddress, tokensToUnfreeze);
        }
        super._burn(_userAddress, _mainId, _subId, _amount);
    ...
    function freezePartialTokens(address _userAddress, uint256 _amount) public override onlyAgent {
        uint256 balance = super.balanceOf(_userAddress);
